...
async function cmd_init(configFilePath, { logger }) {
  ...
  ${chalk.bold.green(
    'Spotivy'
  )} is a command line tool to download Spotify tracks in batch.

  There are basically two commands, one to ${chalk.bold(
    'download a specific track'
  )} (${chalk.bold.gray('spotivy track <track_id>')})
and one to ${chalk.bold('download a whole playlist')} (${chalk.bold.gray(
      'spotivy playlist <user_id> <playlist_id>'
    )}).
  Before downloading any track, we need to configure the program with the keys of both ${chalk.green(
    'Spotify'
  )}
and ${chalk.bold.red('YouTube')} APIs.
  ...
  - ${chalk.bold.red('YouTube:')} Generate an ${chalk.bold(
      'API key'
  ...
  - ${chalk.green(
    'Spotify:'
  )} Create a new application in Spotify developers portal:
             ${chalk.cyan.bold(
               'https://developer.spotify.com/dashboard/applications'
             )}.
             They will provide both ${chalk.bold('Client ID')} and ${chalk.bold(
      'Client Secret'
    )}. Keep them.

  For more detailed information, type ${chalk.bold.gray(
    'spotivy --help'
  )} or head to https://danguilherme.github.io/blog/spotivy/ (pt-br).
